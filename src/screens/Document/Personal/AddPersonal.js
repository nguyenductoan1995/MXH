...
import { addDocument, getListCompanyPersonal } from 'store/Personal/actions'

class AddPersonal extends React.PureComponent {
  constructor(props) {
    super(props)
    this.state = {
      Description: null,
  ...
  get getType() {
    const { optionStore } = this.props
    const { GroupID } = this.state
    const data = get(optionStore, `DocumentType${GroupID}`, [])
    const newData = [{ id: 0, name: 'None' }]
    data.map((item) => {
      newData.push({
        id: get(item, 'Key'),
  ...
  onSelectGroup = (item) => {
    ...
    doGetType({ OptionType: `DocumentType${GroupID}` }, () => { this.setState({ selected: undefined }) })
  }
        ...
        <Block flex={1} style={{}}>
          <ScrollView style={{ flex: 1 }} contentContainerStyle={styles.scrollWrap}>
            <Block center style={styles.main}>
              <Input onChangeText={(value) => this.handleInput('Description', value)} placeholder="Document Title*" style={styles.input} />
              <SelectList
              ...
              <SelectList
                ...
                selected={selected}
              />
              <DatePickers onDateSelect={(value) => this.handleInput('ReceviedOn', value)} placeholder="Document Date*" style={[styles.input, styles.inputlast]} />
              <DatePickers onDateSelect={(value) => this.handleInput('ExpiryDate', value)} placeholder="Expiry Date*" style={[styles.input, styles.inputlast]} />
              <Inputmultiline
                style={styles.input}
...
const mapStateToProps = ({ optionStore }) => {
...
const mapDispatchToProps = (dispatch) => ({
  ...
  doAddData: (evt, cb) => dispatch(addDocument(evt, cb)),
  doGetRates: (evt, cb) => dispatch(getRates(evt, cb)),
  onGetData: (evt, cb) => dispatch(getListCompanyPersonal(evt, cb)),
})


export default connect(mapStateToProps, mapDispatchToProps)(AddPersonal)
