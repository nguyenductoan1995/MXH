...
import { getSMPDetail } from 'store/Profile/actions'

class SMPDetail extends React.PureComponent {
  componentDidMount() {
  ...
  render() {
    ...
    return (
      <Block flex={1} style={styles.contain}>
        <Block>
          <Header onPressLeftIcon={this.goBack} title="SMP Detail" />
        </Block>
        <Block flex={1} middle>
...
export default connect(mapStateToProps, mapDispatchToProps)(SMPDetail)
