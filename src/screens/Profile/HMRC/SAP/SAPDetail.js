...
class SAPDetail extends React.PureComponent {
  componentDidMount() {
  ...
  render() {
    ...
    return (
      <Block flex={1} style={styles.contain}>
        <Block>
          <Header onPressLeftIcon={this.goBack} title="SAP Detail" />
        </Block>
        <Block flex={1} middle>
...
const mapStateToProps = ({ profileStore }) => {
  const data = get(profileStore, 'SAPDetail', {})
  return {
    data,
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(SAPDetail)
